...
document.getElementById("gender-div").addEventListener("click", (e) => {
  ...
  document
    .querySelectorAll("#gender-div button")
...
document.getElementById("age-div").addEventListener("click", (e) => {
  ...
  document
    .querySelectorAll("#age-div button")
    .forEach((b) => b.classList.remove("active"));
  e.target.classList.add("active");
  getScores();
  setScores();
...
function setupExempt(checkbox, overlaySelector) {
  checkbox.addEventListener("change", (e) => {
    ...
    e.target
      .closest(".comp-header")
      .classList.toggle("is-exempt", e.target.checked);
    getScores();
    setScores();
  });
}
...
const coreConfig = {
  ...
  "cross-leg": {
    exercise: "crossLegReverseCrunch",
    placeholder: "reps",
    label: "Reps in 2 min",
  },
...
document.getElementById("cardio-btns-div").addEventListener("click", (e) => {
  ...
  if (!cfg) return;
  document
    .querySelectorAll("#cardio-btns-div button")
    .forEach((b) => b.classList.remove("active"));
  e.target.classList.add("active");
...
document.getElementById("strength-btns-div").addEventListener("click", (e) => {
  ...
  if (!cfg) return;
  document
    .querySelectorAll("#strength-btns-div button")
    .forEach((b) => b.classList.remove("active"));
  e.target.classList.add("active");
...
document.getElementById("core-btns-div").addEventListener("click", (e) => {
  ...
  if (!cfg) return;
  document
    .querySelectorAll("#core-btns-div button")
    .forEach((b) => b.classList.remove("active"));
  e.target.classList.add("active");
...
[cardioInput, strengthInput, coreInput, waistInput, heightInput].forEach(
  (input) => {
    input.addEventListener("change", () => {
      validateAndShow(input);
      getScores();
      setScores();
    });
...
const validationRules = {
  twoMile: {
    pattern: /^\d{4}$/,
    extraCheck: (v) => Number(v) >= 1000,
    msg: "Enter 4 digits (e.g. 1430 = 14:30)",
  },
...
function validateAndShow(inputEl) {
  ...
  const valid =
    rule.pattern.test(val) && (!rule.extraCheck || rule.extraCheck(val));
...
function lookupPoints(entries, score, lowerIsBetter = false) {
  const match = entries.find(
    lowerIsBetter ? (t) => score <= t.threshold : (t) => score >= t.threshold,
  );
...
function getScores() {
  const {
    gender,
    ageGroup: age,
    cardioExercise: cardioEx,
    strengthExercise: strengthEx,
    coreExercise: coreEx,
  } = userProfile;
  ...
  if (exemptCardio.checked) {
    ...
    const cardioScore =
      cardioEx === "twoMile" ? convertTime(cardioRaw) : cardioRaw;
    userProfile.cardioScore = cardioScore;
    userProfile.cardioPoints = lookupPoints(
      table[cardioEx],
      cardioScore,
      cardioEx === "twoMile",
    );
  ...
  if (exemptBodyComp.checked) {
    ...
    const ratio = (
      Number(waistInput.value.trim()) / Number(heightInput.value.trim())
    ).toFixed(2);
  ...
  userProfile.totalPoints = Math.floor(
...
function setMinMax() {
  ...
  function updateBar(
    ...
    document.getElementById(minSpanId).textContent =
      `Min: ${isTime ? formatTime(worst) : worst}`;
    document.getElementById(maxSpanId).textContent =
      `Max: ${isTime ? formatTime(best) : best}`;
    let percent = 0;
    if (userScore > 0) {
...
function setScores() {
  ...
  document.getElementById("cardio-ex-label").textContent =
    exerciseDisplayNames[cardioEx];
  document.getElementById("strength-ex-label").textContent =
    exerciseDisplayNames[strengthEx];
  document.getElementById("core-ex-label").textContent =
    exerciseDisplayNames[coreEx];

  // Compute per-component statuses
  const cardioSt = componentStatus(
    cardioInput,
    userProfile.cardioScore,
    table[cardioEx],
    cardioEx === "twoMile",
    exemptCardio.checked,
  );
  const strengthSt = componentStatus(
    strengthInput,
    userProfile.strengthScore,
    table[strengthEx],
    false,
    exemptStrength.checked,
  );
  const coreSt = componentStatus(
    coreInput,
    userProfile.coreScore,
    table[coreEx],
    false,
  ...
  const whtrSt = exemptBodyComp.checked
    ? "exempt"
    : whtrEntered
      ? Number(userProfile.WHtRScore) <=
        WHtRTable[WHtRTable.length - 1].threshold
        ? "pass"
        : "fail"
      : null;
  ...
  updateBreakdownRow(
    "cardio",
    "cardio-status",
    userProfile.cardioPoints,
    50,
    cardioSt,
    cardioInput.value.trim(),
  );
  updateBreakdownRow(
    "strength",
    "strength-status",
    userProfile.strengthPoints,
    15,
    strengthSt,
  ...
  updateBreakdownRow(
    "whtr",
    "whtr-status",
    userProfile.WHtRPoints,
    20,
    whtrSt,
    whtrEntered,
  );

  const anyInputEntered =
    exemptCardio.checked ||
    exemptStrength.checked ||
    exemptCore.checked ||
    exemptBodyComp.checked ||
    cardioInput.value.trim() ||
    strengthInput.value.trim() ||
    coreInput.value.trim() ||
    whtrEntered;
...
function componentStatus(inputEl, score, entries, lowerIsBetter, isExempt) {
  ...
  return (lowerIsBetter ? score <= minThreshold : score >= minThreshold)
    ? "pass"
    : "fail";
}
...
function getScoreRating() {
  ...
  const allPass = [cardioSt, strengthSt, coreSt, whtrSt].every(
    (s) => s !== "fail",
  );
